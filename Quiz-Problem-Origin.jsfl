 ...
 * Global variables
 */
var allFiles, directory, file, frames, labelLayerIndex, labelFrames, layers, currentLesson, logText, logURI,
    originLayerIndex,
    origins, problems;
...
function main() {
    ...
    allFiles = FLfile.listFolder(directory, "files");
    logURI = directory + "lesson-problem-origin.txt";
    logText = "lesson, problem, origin";

    write(logURI.toString(), logText, true);
    log("=> Column labels written");

    allFiles = allFiles.slice(1);
